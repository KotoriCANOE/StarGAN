...
class Train:
    ...
    def get_dataset(self):
        ...
        if self.data.num_domains is not None:
            self.config.num_domains = self.data.num_domains
            eprint('--num-domains is set to {}'.format(self.config.num_domains))
        ...
        self.val_origins = []
        self.val_targets = []
        for _inputs, _origins, _targets in self.data.gen_val():
            self.val_inputs.append(_inputs)
            self.val_origins.append(_origins)
            self.val_targets.append(_targets)
    ...
    def build_graph(self):
        with tf.device(self.device):
            ...
            self.g_train_summary, self.d_train_summary, self.loss_summary = self.model.get_summaries()
